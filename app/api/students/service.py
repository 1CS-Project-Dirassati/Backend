...
class StudentService:
    ...
    # --- GET Single ---
    @staticmethod
    def get_student_data(student_id: int, current_user_id: int, current_user_role: str, current_user_parent_id=None):
    ...
    def get_all_students(
        ...
        try:
            ...
            filters_applied = {}
            if level_id is not None: filters_applied['level_id'] = level_id
            if group_id is not None: filters_applied['group_id'] = group_id
            if parent_id is not None and current_user_role in ["admin", "teacher"]: filters_applied['parent_id'] = parent_id
            if is_approved is not None: filters_applied['is_approved'] = is_approved
    ...
    def create_student(data: dict):
        ...
        try:
            ...
            current_app.logger.debug(f"Admin creating student directly. Validating FKs.")
            fk_errors = StudentService._validate_foreign_keys(new_student) # Pass the instance
            if fk_errors:
            ...
            db.session.commit()
            current_app.logger.info(f"Student created successfully by admin with ID: {new_student.id}")
    ...
    def add_child(data: dict, parent_id: int):
        ...
        try:
            ...
            current_app.logger.info(f"Parent {parent_id} initiating registration for child with email {student_email}")
            current_app.logger.debug(f"Received add_child data: {data}") # Log received data
            ...
            if existing_student:
                current_app.logger.warning(f"Attempt to add child failed: Email '{student_email}' already exists for student ID {existing_student.id}.")
                return err_resp(f"A student account with the email '{student_email}' already exists.", "duplicate_email", 409)

            # 3. Check if registration is already pending in Redis
            redis_key = f"child_reg_pending:{student_email}"
            if redis_client.exists(redis_key):
                ttl = redis_client.ttl(redis_key)
                current_app.logger.warning(f"Attempt to add child failed: Registration already pending for email '{student_email}'. TTL: {ttl}s")
                return err_resp(f"A registration invitation has already been sent recently to {student_email}. Please check the inbox or wait.", "registration_pending", 429)
            ...
            redis_expiry_seconds = current_app.config.get("CHILD_REGISTRATION_TOKEN_MAX_AGE_SECONDS", 7200)

            # 5. Store Data in Redis
            try:
                redis_client.set(redis_key, json.dumps(redis_data), ex=redis_expiry_seconds)
                current_app.logger.debug(f"Stored pending child registration data in Redis for {student_email} with expiry {redis_expiry_seconds}s.")
            except Exception as redis_err:
                 current_app.logger.error(f"Failed to store child registration data in Redis for {student_email}: {redis_err}", exc_info=True)
                 return internal_err_resp()
            ...
            try:
                ...
                current_app.logger.debug(f"Generated child registration token for email {student_email}")
            except Exception as e:
                current_app.logger.error(f"Failed to serialize child registration token for {student_email}: {e}", exc_info=True)
                try: redis_client.delete(redis_key) # Cleanup Redis
                except: pass
                return internal_err_resp()
            ...
            if not frontend_base:
                current_app.logger.error("FRONTEND_BASE_URL is not configured. Cannot create child registration link.")
                try: redis_client.delete(redis_key) # Cleanup Redis
                except: pass
                return internal_err_resp()

            registration_path = current_app.config.get("FRONTEND_CHILD_REGISTRATION_PATH", "/complete-child-registration")
            ...
            template = "email/child_registration"
            expiration_minutes = int(redis_expiry_seconds / 60)
            parent_obj = Parent.query.get(parent_id) # Fetch parent object
            parent_name = parent_obj.first_name if parent_obj else "Your Parent" # Get name safely
            context = {
            ...
            if not email_sent:
                current_app.logger.error(f"Failed to send child registration email to {student_email}. Data remains in Redis.")
                return message(True, "Registration invitation sent, but email delivery failed. Please contact support if the child doesn't receive it."), 200

            # 9. Return Success Response to Parent
            current_app.logger.info(f"Child registration email successfully sent to {student_email} initiated by parent {parent_id}.")
            return message(True, "Registration invitation email sent to the child."), 200

        except ValueError as ve: # Catch config errors
            current_app.logger.critical(f"Configuration error during add child process: {ve}")
            return internal_err_resp()
        except KeyError as ke: # Catch if expected keys are missing from data
             current_app.logger.error(f"Missing expected key in add_child data: {ke}. Data: {data}", exc_info=True)
            ...
            try: redis_client.delete(redis_key) # Attempt cleanup
            except: pass
            return internal_err_resp()
    # -----------------------------------------------------
    @staticmethod
    def complete_child_registration(data: dict):
        ...
        try:
            ...
            serializer = StudentService._get_child_registration_serializer()
            max_age = current_app.config.get("CHILD_REGISTRATION_TOKEN_MAX_AGE_SECONDS", 7200)
            try:
                if not token:
                ...
                redis_key = f"child_reg_pending:{student_email}" # Define key here for cleanup
                current_app.logger.debug(f"Child registration token validated for email: {student_email}")
            except SignatureExpired:
                current_app.logger.warning(f"Expired child registration token received.")
                return err_resp("Registration link has expired.", "token_expired", 400)
            except BadSignature as e:
                current_app.logger.warning(f"Invalid child registration token received: {e}")
            ...
            try:
                ...
                if not redis_data_json:
                    current_app.logger.warning(f"No pending registration data found in Redis for key: {redis_key} (likely expired/completed).")
                    # Use 404 Not Found as the resource (pending registration) is gone
                    return err_resp("Registration data not found. It may have expired or already been used.", "reg_data_not_found", 404)
                redis_data = json.loads(redis_data_json)
                current_app.logger.debug(f"Retrieved pending registration data from Redis for {student_email}")
            except json.JSONDecodeError:
                current_app.logger.error(f"Failed to decode JSON registration data from Redis for key: {redis_key}")
                return internal_err_resp() # Internal state error
            except Exception as redis_err:
                 current_app.logger.error(f"Failed to retrieve child registration data from Redis for {redis_key}: {redis_err}", exc_info=True)
                 return internal_err_resp()
            ...
            try:
                ...
                data['is_approved'] = False # Default to unapproved
                 ...
                 current_app.logger.error(f"Missing expected key in Redis data for {redis_key}: {ke}")
                 return internal_err_resp() # Internal state error

            # 3. Check for Existing Student (Race Condition)
            existing_student = Student.query.filter_by(email=student_email).first()
            if existing_student:
                current_app.logger.warning(f"Conflict during child registration completion: Email '{student_email}' already exists for student ID {existing_student.id}.")
                # Attempt to clean up Redis key if found during race condition check
                try: redis_client.delete(redis_key)
                except: pass
                return err_resp(f"A student account with the email '{student_email}' was created after the invitation was sent.", "duplicate_email_concurrent", 409)

            # 4. Hash Password
            # Basic length validation done by DTO/expect
            hashed_password = generate_password_hash(redis_data['password'])
            data['password'] = hashed_password
            current_app.logger.debug(f"Password hashed for completing registration for {student_email}")
            ...
            db.session.add(new_student)
            db.session.commit()
            current_app.logger.info(f"Student record created successfully upon registration completion for email {student_email}, ID: {new_student.id}")

            # 6. Delete Redis Data (Cleanup) - After successful commit
            try:
                deleted_count = redis_client.delete(redis_key)
                if deleted_count > 0:
                    current_app.logger.debug(f"Successfully deleted pending registration data from Redis for key: {redis_key}")
                else:
                     current_app.logger.warning(f"Attempted to delete Redis key {redis_key} after registration, but key was not found.")
            except Exception as redis_del_err:
                # Log error but don't fail the request, registration is complete
                current_app.logger.error(f"Failed to delete Redis key {redis_key} after successful registration: {redis_del_err}", exc_info=True)
            ...
            access_token = create_access_token(
                ...
                expires_delta=timedelta(seconds=current_app.config["ACCESS_EXPIRES_SECONDS"])
            )
            refresh_token = create_refresh_token(
                identity=identity,
                additional_claims=additional_claims,
                expires_delta=timedelta(days=current_app.config["REFRESH_EXPIRES_DAYS"])
            )
            current_app.logger.info(f"Generated login tokens for newly registered student {new_student.id}")
            ...
            resp["user"] = student_resp_data # Use 'user' key consistent with auth responses
            return resp, 201 # 201 Created

        except ValueError as ve: # Catch config errors
            current_app.logger.critical(f"Configuration error during complete child registration: {ve}")
            return internal_err_resp()
        except IntegrityError as error: # Catch potential DB unique constraint issues (should be caught by earlier check)
            db.session.rollback()
            current_app.logger.error(f"Database integrity error during complete child registration: {error}", exc_info=True)
            return internal_err_resp() # Unexpected if pre-check worked
        except SQLAlchemyError as error:
            db.session.rollback()
            current_app.logger.error(f"Database error during complete child registration: {error}", exc_info=True)
            return internal_err_resp()
        except Exception as error:
            db.session.rollback()
            current_app.logger.error(f"Unexpected error during complete child registration: {error}", exc_info=True)
    ...
    def update_student(student_id: int, data: dict):
        ...
        try:
            ...
            current_app.logger.debug(f"Student data validated by schema for update. Committing changes for ID: {student_id}")
    ...
    def update_approval_status(student_id: int, data: dict):
        ...
        try:
            ...
            if 'is_approved' not in data or not isinstance(data['is_approved'], bool):
                 ...
                 # Or simpler err_resp
                 return err_resp("Missing or invalid 'is_approved' field.", "validation_error", 400)
            ...
            if student.is_approved != new_status:
                ...
                current_app.logger.info(f"Student {student_id} approval status already {new_status}. No update needed.")
    ...
    def delete_student(student_id: int):
        ...
        try:
            current_app.logger.warning(f"Admin attempting to delete student ID: {student_id}")
