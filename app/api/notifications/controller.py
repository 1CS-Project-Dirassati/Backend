...
from .service import NotificationApiService
...
unread_count_resp = NotificationDto.unread_count_resp
# This DTO is now simplified and does not expect 'type' or 'notification_type'
...
def get_current_user_info():
    user_id = get_jwt_identity()  # This is User.id
    claims = get_jwt()
    role = claims.get("role")  # e.g., "admin", "student"
    if not role:
...
# --- Route for current user's notifications ---
@api.route("/me")
class MyNotificationList(Resource):
    ...
    @api.doc(
        ...
        security="Bearer",
        parser=notification_filter_parser,  # Uses the updated parser
        description="Get a paginated list of notifications for the currently logged-in user. Filterable by read status and notification type.",
        responses={
    ...
    @limiter.limit(
        lambda: current_app.config.get("RATE_LIMIT_NOTIFICATION_LIST", "60/minute")
    )
    def get(self):
        ...
        return NotificationApiService.get_my_notifications(
            ...
            notification_type_filter_str=args.get(
                "notification_type"
            ),  # Use 'notification_type' from parser
            page=args.get("page"),
...
class NotificationAdminCreate(Resource):
    ...
    @api.doc(
        ...
        security="Bearer",
        description="Create a notification targeted at a specific user. 'notification_type' defaults to 'system'.",
        responses={
            201: ("Created", data_resp),
            400: "Validation Error/Invalid Data",
            401: "Unauthorized",
            403: "Forbidden (Not Admin or role missing)",
            404: "Not Found (Recipient user not found)",
            429: "Too Many Requests",
            500: "Internal Server Error",
        },
    )
    # notification_create_input DTO no longer expects 'type' or 'notification_type'
    @api.expect(notification_create_input, validate=True)
    @jwt_required()
    @roles_required("admin")
    @limiter.limit(
        lambda: current_app.config.get(
            "RATE_LIMIT_NOTIFICATION_ADMIN_CREATE", "20/minute"
        )
    )
    def post(self):
        """Create a new notification (Admin only, defaults to SYSTEM type)"""
        data = request.get_json()
        # Data will NOT contain 'type' or 'notification_type' due to DTO change.
        try:
...
class NotificationResource(Resource):
    ...
    @roles_required("parent", "student", "teacher", "admin")
    @limiter.limit(
        lambda: current_app.config.get("RATE_LIMIT_NOTIFICATION_GET", "100/minute")
    )
    def get(self, notification_id: int):
        ...
        try:
            ...
            return {"status": False, "message": str(e)}, 403
        return NotificationApiService.get_notification_data(
            notification_id, user_id, role
        )
    ...
    @roles_required("parent", "student", "teacher", "admin")
    @limiter.limit(
        lambda: current_app.config.get("RATE_LIMIT_NOTIFICATION_UPDATE", "80/minute")
    )
    ...
    @roles_required("parent", "student", "teacher", "admin")
    @limiter.limit(
        lambda: current_app.config.get("RATE_LIMIT_NOTIFICATION_DELETE", "50/minute")
    )
    def delete(self, notification_id: int):
        ...
        try:
            ...
            return {"status": False, "message": str(e)}, 403
        return NotificationApiService.delete_notification(
            notification_id, user_id, role
        )
...
class NotificationReadAll(Resource):
    ...
    @roles_required("parent", "student", "teacher", "admin")
    @limiter.limit(
        lambda: current_app.config.get("RATE_LIMIT_NOTIFICATION_READ_ALL", "10/minute")
    )
    def post(self):
        """Mark all my notifications as read"""
        try:
...
class NotificationUnreadCount(Resource):
    ...
    @limiter.limit(
        lambda: current_app.config.get(
            "RATE_LIMIT_NOTIFICATION_UNREAD_COUNT", "60/minute"
        )
    )
    def get(self):
        """Get my unread notification count"""
